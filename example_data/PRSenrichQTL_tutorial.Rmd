...
if(file.exists("c5.bp.v7.1.symbols.gmt")){
  ...
  print("Let's select the top three candidates that appear more than 10 times:")
  as.character(lead[lead$Freq>10, "Var1"])
}
```

## Run trans eQTL analysis

Let's run a trans eQTL analysis for those three genes and the 109 AF GWAS SNPs:

```{r, message=FALSE}
if(file.exists("c5.bp.v7.1.symbols.gmt")){
  eQTL <- trans.core <- trans.qtl(prefix = "/tmp/example",
                                  genotype_file_name = "1k_genomes_AF_snps.txt",
                                  expression_file_name = t(df[, c("CD28", "IL10", "TNFSF4")]),
