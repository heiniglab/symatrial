...
```{r}
# eQTS <- data.frame(matrix(nrow = dim(texpr)[1],
#                           ncol = 6),
#                    stringsAsFactors = F,
#                               row.names = colnames())
#   colnames(res.GPS.trans) <- c("id", "Estimate", "StdError", "tvalue", "pvalueT", "padjT")
# data <- df[, c("id", "PRS.AF", "PRS.AF.percentile", "PRS.CAD", "PRS.CAD.percentile", "sex", "population")]
# for (i in 8:(dim(df)[2])){
#     #i=1
#     id <- colnames(df)[i]
#     data$expr <- df[, id]
