...
```{r, echo=F}
packages <- c("devtools", "fgsea", "MatrixEQTL", "eQTLpipeline")
packages <- packages[!(packages %in% rownames(installed.packages()))]
if(length(packages)>0){
  print("There are additional packages that need to be installed to follow this tutorial. You can change the `eval` option in the next code chunk from `F` to `T` to install the missing packages")
}
```
