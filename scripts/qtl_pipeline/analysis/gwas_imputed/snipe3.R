...
snipa.base              <-"https://snipa.helmholtz-muenchen.de/snipa3"
...
snipa.get.rand <- function() {
  ...
  stop_for_status(s)
  #Updated due to deprecation of html()
...
snipa.submit.query <- function(params, uri, paging=T) {
  ...
  if (params$snps_input_type == 'snps') {
    if(paging) {
      ...
      if(length(params$snps_sentinels)> paging.length.threshold){
        warning('Number of SNPs is above the threshold! Disable pairwise or decrease the number...')
      }
      params$id <- snipa.get.rand()
      params$snps_sentinels <- paste(params$snps_sentinels, collapse = '\n')
